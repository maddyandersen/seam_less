...
def home():
    ...
    if "username" in session:
        ...
        return render_template('profile.html', user_id=session['user_id'], fName= session["first_name"], lName= session["last_name"], name=session['username'], password=session['password'], blogs=list_blogs(session["user_id"]), access = True)
...
def user():
    ...
    if len(check) == 0:
        ...
        return render_template('error.html', message="Username already exists", route="/register")
    ...
    session["visitor_id"] = session["user_id"]
    return render_template('profile.html', fName=first_name, lName=last_name, name=username, password=password, user_id=user_id, access = True)
...
def show_existing_blog():
    ...
    return render_template('blog.html', blog_id=blog_id, name=data[0], description=data[1], username=session["username"], entries=list_entries(session["blog_id"]), access = access)
...
def entry_save():
    ...
    create_entry(entry_id, session["blog_id"], entry_title, content)
    return redirect(url_for('show_existing_blog'))

#methods related to viewing other blogs
...
def view_blogs():
    ...
    data = c.fetchone()
    return render_template('profile.html', fName=data[1], lName=data[2], name=data[0], user_id=session["visitor_id"], blogs=list_blogs(session["visitor_id"]), access=False)
...
def authenticate():
    ...
    if len(data) == 0:
        return render_template('error.html', message="Login Failed", route="/")
    for d in data:
        user_id = d[0]
        first_name = d[1]
        last_name = d[2]
    ...
    session["last_name"] = last_name
    session["visitor_id"] = session["user_id"]
    return render_template('profile.html', user_id=user_id, name=username, fName=first_name, lName=last_name, password=password, data=list_blogs(session["user_id"]), blogs=list_blogs(user_id), access = True)  #response to a form submission
